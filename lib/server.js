...
server.unifiedServer = function(req, res) {
  ...
  req.on('end', function(){
    buffer += decoder.end();
    debug('Requested payload: '+buffer);
...
server.init = function() {
    ...
    console.log('\x1b[36m%s\x1b[0m','The server is listening on port '+config.httpPort+' in '+config.envName+' environment now');
  });

  //Starts HTTPS server
  //The server should listen on a httpsport
    server.httpsServer.listen(config.httpsPort, function(){
    console.log('\x1b[35m%s\x1b[0m','The server is listening on port '+config.httpsPort+' in '+config.envName+' environment now');
  });
}
